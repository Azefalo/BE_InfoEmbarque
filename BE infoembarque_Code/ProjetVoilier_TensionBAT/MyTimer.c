...
//	  MyGPIO_Toggle(led.GPIO,led.GPIO_Pin);
    ...
    if (Timer_Callback) {
    Timer_Callback(); // Appeler la fonction définie par l'utilisateur
    }
}
...
//		
	  if(Channel==1){
			...
			Timer->CCMR1 |= TIM_CCMR1_OC1PE;          // Activer le préchargement pour la comparaison
		...
		if(Channel==2){
			...
			Timer->CCMR1 |= (0x6 << 12);  // Régler le mode PWM 1 (0x6)
			Timer->CCMR1 |= TIM_CCMR1_OC2PE;          // Activer le préchargement pour la comparaison
		...
		if(Channel==3){
			...
			Timer->CCMR2 |= (0x6 << 4);  // Régler le mode PWM 1 (0x6)
			Timer->CCMR2 |= TIM_CCMR2_OC3PE;          // Activer le préchargement pour la comparaison
		...
		if(Channel==4){
			...
			Timer->CCMR2 |= (0x6 << 12);  // Régler le mode PWM 1 (0x6)
			Timer->CCMR2 |= TIM_CCMR2_OC4PE;          // Activer le préchargement pour la comparaison
